...
while(True):
    ...
    if(opcion == 1):
        system("clear")
        recorrido_total = 0
        print("Las distancias entre nodos son las siguientes")
        ...
        for arista in arbol_aristas:
            G.add_edge(nodos[arista[0]], nodos[arista[1]], weight=arista[2])
            
            print("* La distancia del nodo", nodos[arista[0]], "al nodo", nodos[arista[1]], "es", arista[2])

        # Mostrando grafo
        nx.draw(G, with_labels=True, font_color="white", font_size=20, node_color="black", node_size=1000)
        plt.margins(0.2)
        plt.show()
